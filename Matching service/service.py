...
def age_gap(age_frames: str) -> List[int]:
    """
    ...
    return list(range(minimal_age, maximal_age + 1))
    ...
    for pref_age in age_gap(userdata1['preferred_age']):
        ...
        if matched_user_id:
            if matched_user_id != request.uid:
                return {'status': 'success', 'message': 'user found', 'uid': matched_user_id}

    queue_key = f"queue:{userdata1['age']}-{userdata1['sex']}"
    if await add_user_to_queue(request.uid, queue_key):
